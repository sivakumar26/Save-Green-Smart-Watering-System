...
$query = "SELECT sensor_value, to_char(time, 'DD/MM/YYYY') FROM finaltable WHERE sensor_value IN (SELECT MIN(sensor_value) FROM finaltable WHERE to_char(time, 'DD/MM/YYYY') IN (SELECT DISTINCT to_char(time, 'DD/MM/YYYY') FROM finaltable) GROUP BY to_char(time, 'DD/MM/YYYY')) ORDER BY auto DESC LIMIT 5";
